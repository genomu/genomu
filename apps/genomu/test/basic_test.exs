...
defmodule Genomu.BasicTest do
  ...
  test "channel retrieving a key at a specified version", context do
    ...
    {:ok, ch} = C.begin(conn)
    {"123", vsn} = C.set(ch, ["some"], API.Core.identity("123"), version: true)
    C.commit(ch)

    {:ok, ch} = C.begin(conn)
    assert C.get(ch, {["some"], vsn}, API.Core.identity, version: true) == {"123", vsn}
  end
  ...
  test "channel retrieving a key at a specified upper bound version", context do
    ...
    {:ok, ch} = C.begin(conn)
    {"123", vsn0} = C.set(ch, ["some1"], API.Core.identity("123"), version: true)
    {"123", vsn} = C.set(ch, ["some2"], API.Core.identity("123"), version: true)
    C.commit(ch)

    {:ok, ch} = C.begin(conn)
    assert C.get(ch, {["some1"], vsn}, API.Core.identity, version: true, txn: true) == {"123", vsn0, vsn}
  end
  ...
  test "channel retrieving an operation at a specified version", context do
    ...
    {:ok, ch} = C.begin(conn)
    {"123", vsn} = C.set(ch, ["someop1"], API.Core.identity("123"), version: true)
    C.commit(ch)

    {:ok, ch} = C.begin(conn)
    assert C.operation(ch, {["someop1"], vsn}, version: true) == {{API.Core, :identity, ["123"]}, vsn}
  end
  ...
  test "channel retrieving an operation at a specified upper bound version", context do
    ...
    {:ok, ch} = C.begin(conn)
    {"123", vsn0} = C.set(ch, ["someop1x"], API.Core.identity("123"), version: true)
    {"123", vsn} = C.set(ch, ["someop2x"], API.Core.identity("123"), version: true)
    C.commit(ch)

    {:ok, ch} = C.begin(conn)
    assert C.operation(ch, {["someop1x"], vsn}, version: true, txn: true) == {{API.Core, :identity, ["123"]}, vsn0, vsn}
  end
